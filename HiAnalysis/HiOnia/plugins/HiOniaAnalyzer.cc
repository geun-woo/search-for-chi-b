...
//
/**\class HiOniaAnalyzer HiOniaAnalyzer.cc
 UserCode/tdahms/HiAnalysis/HiOnia/plugins/HiOniaAnalyzer.cc
...
class HiOniaAnalyzer : public edm::EDAnalyzer {
   ...
   private:
    ...
    enum dimuonCategories { GlbTrk_GlbTrk = 0, Glb_Glb = 1, Trk_Trk = 2 };

    enum muonCategories { GlbTrk = 0, Trk = 1, Glb = 2 };
    ...
    int Reco_conv_size;
    double Reco_conv_chi2_probability[Max_conv_size];
    int Reco_conv_ntracks[Max_conv_size];
    double Reco_conv_dz_innerPosition[Max_conv_size];
    double Reco_conv_dPhiTracksAtVtx[Max_conv_size];
    double Reco_conv_pairCotThetaSeparation[Max_conv_size];
    double Reco_conv_distOfMinimumApproach[Max_conv_size];
    double Reco_conv_zOfPrimaryVertexFromTracks[Max_conv_size];
    double Reco_conv_pairInvariantMass[Max_conv_size];
    //
    int Reco_conv_trk1_charge[Max_conv_size];
    int Reco_conv_trk1_nhit[Max_conv_size];
    double Reco_conv_trk1_normalizedChi2[Max_conv_size];
    double Reco_conv_trk1_theta[Max_conv_size];
    double Reco_conv_trk1_phi[Max_conv_size];
    double Reco_conv_trk1_d0[Max_conv_size];
    //
    int Reco_conv_trk2_charge[Max_conv_size];
    int Reco_conv_trk2_nhit[Max_conv_size];
    double Reco_conv_trk2_normalizedChi2[Max_conv_size];
    double Reco_conv_trk2_theta[Max_conv_size];
    double Reco_conv_trk2_phi[Max_conv_size];
    double Reco_conv_trk2_d0[Max_conv_size];
    //
    double Reco_conv_pt[Max_conv_size];
    double Reco_conv_eta[Max_conv_size];
    double Reco_conv_phi[Max_conv_size];
    double Reco_conv_et[Max_conv_size];
    double Reco_conv_energy[Max_conv_size];
    double Reco_conv_deltaPhi[Max_conv_size];
    double Reco_conv_deltaEta[Max_conv_size];
    double Reco_conv_deltaR[Max_conv_size];
    ...
    int Reco_QQ_mumi_nPixValHits[Max_QQ_size];  // Number of valid pixel hits in
                                                // minus sta muons
    int Reco_QQ_mupl_nMuValHits[Max_QQ_size];   // Number of valid muon hits in
                                                // plus sta muons
    int Reco_QQ_mumi_nMuValHits[Max_QQ_size];   // Number of valid muon hits in
                                                // minus sta muons
    ...
    int Reco_QQ_mupl_nPixWMea[Max_QQ_size];     // pixel layers with measurement
                                                // for plus inner track muons
    int Reco_QQ_mumi_nPixWMea[Max_QQ_size];     // pixel layers with measurement
                                                // for minus inner track muons
    int Reco_QQ_mupl_nTrkWMea[Max_QQ_size];     // track layers with measurement
                                                // for plus inner track muons
    int Reco_QQ_mumi_nTrkWMea[Max_QQ_size];     // track layers with measurement
                                                // for minus inner track muons
    int Reco_QQ_mupl_StationsMatched
        [Max_QQ_size];  // number of stations matched for plus inner track muons
    int Reco_QQ_mumi_StationsMatched[Max_QQ_size];   // number of stations
                                                     // matched for minus inner
                                                     // track muons
    float Reco_QQ_mupl_normChi2_inner[Max_QQ_size];  // chi2/ndof for plus inner
                                                     // track muons
    float Reco_QQ_mumi_normChi2_inner[Max_QQ_size];  // chi2/ndof for minus
                                                     // inner track muons
    float Reco_QQ_mupl_normChi2_global[Max_QQ_size];  // chi2/ndof for plus
                                                      // global muons
    float Reco_QQ_mumi_normChi2_global[Max_QQ_size];  // chi2/ndof for minus
                                                      // global muons
    ...
    float Reco_QQ_mupl_dxyErr[Max_QQ_size];  // dxy error for plus inner track
                                             // muons
    float Reco_QQ_mumi_dxyErr[Max_QQ_size];  // dxy error for minus inner track
                                             // muons
    ...
    float
        Reco_QQ_mupl_dzErr[Max_QQ_size];  // dz error for plus inner track muons
    float Reco_QQ_mumi_dzErr[Max_QQ_size];     // dz error for minus inner track
                                               // muons
    ...
    float Reco_QQ_mupl_ptErr_inner[Max_QQ_size];   // pT error for plus inner
                                                   // track muons
    float Reco_QQ_mumi_ptErr_inner[Max_QQ_size];   // pT error for minus inner
                                                   // track muons
    float Reco_QQ_mupl_ptErr_global[Max_QQ_size];  // pT error for plus global
                                                   // muons
    float Reco_QQ_mumi_ptErr_global[Max_QQ_size];  // pT error for minus global
                                                   // muons

    int Reco_mu_size;  // Number of reconstructed muons
    int Reco_mu_SelectionType[Max_mu_size];
    ULong64_t Reco_mu_trig[Max_mu_size];  // Vector of trigger bits matched to
                                          // the muon
    int Reco_mu_charge[Max_mu_size];      // Vector of charge of muons
    int Reco_mu_type[Max_mu_size];        // Vector of type of muon (global=0,
                                          // tracker=1, calo=2)
    ...
    int Npix, NpixelTracks, Ntracks, NtracksPtCut, NtracksEtaCut,
        NtracksEtaPtCut;
    float SumET_HF, SumET_HFplus, SumET_HFminus, SumET_HFplusEta4,
        SumET_HFminusEta4, SumET_HFhit, SumET_HFhitPlus, SumET_HFhitMinus,
        SumET_EB, SumET_ET, SumET_EE, SumET_EEplus, SumET_EEminus, SumET_ZDC,
        SumET_ZDCplus, SumET_ZDCminus;
    ...
    // handles
    edm::Handle<reco::ConversionCollection> collConv;  // For conv
...
HiOniaAnalyzer::HiOniaAnalyzer(const edm::ParameterSet &iConfig)
    ...
    for (unsigned int iCent = 0; iCent < _centralityranges.size(); ++iCent) {
        if (iCent == 0)
            ...
            centLabel << _centralityranges.at(iCent - 1)
                      << _centralityranges.at(iCent);
    ...
    NTRIGGERS =
        NTRIGGERS_DBL + _sglTriggerPathNames.size() + 1;  // + 1 for "NoTrigger"
    std::cout << "NTRIGGERS_DBL = " << NTRIGGERS_DBL
              << "\t NTRIGGERS_SGL = " << _sglTriggerPathNames.size()
    ...
    for (std::vector<std::string>::iterator it = theTriggerNames.begin();
         it != theTriggerNames.end(); ++it) {
...
void HiOniaAnalyzer::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
    ...
    if ((_isHI || _isPA) && _useEvtPlane) {
        ...
        if (flatEvtPlanes.isValid()) {
            for (reco::EvtPlaneCollection::const_iterator rp = flatEvtPlanes->begin(); rp != flatEvtPlanes->end(); rp++) {
                rpAng[nEP] = rp->angle(2);  // Using Event Plane Level 2 -> Includes
                                            // recentering and flattening.
                rpSin[nEP] = rp->sumSin(2); // Using Event Plane Level 2 -> Includes
                                            // recentering and flattening.
                rpCos[nEP] = rp->sumCos(2); // Using Event Plane Level 2 -> Includes
                                            // recentering and flattening.
                nEP++;
            }
        } else if (!_isMC) {
    ...
    // For conversion
    iEvent.getByToken(_recoConversionToken, collConv);

    Reco_conv_4mom->Clear();
    Reco_conv_vertex->Clear();

    Reco_conv_size = 0;
    if (collConv->size() < Max_conv_size && collConv.isValid()) {
        for (reco::ConversionCollection::const_iterator it = collConv->begin(); it != collConv->end(); ++it) {
            if (!it->conversionVertex().isValid() || it->nTracks() != 2) continue;

            TLorentzVector *conv_4mom_vector4 = (TLorentzVector *)Reco_conv_4mom->ConstructedAt(Reco_conv_size);
            conv_4mom_vector4->SetXYZT(it->refittedPair4Momentum().X(),
                                            it->refittedPair4Momentum().Y(),
                                            it->refittedPair4Momentum().Z(),
                                            it->refittedPair4Momentum().T());

            TVector3 *conv_vertex_vector3 = (TVector3 *)Reco_conv_vertex->ConstructedAt(Reco_conv_size);
            const reco::Vertex conv_vertex = it->conversionVertex();
            conv_vertex_vector3->SetXYZ(conv_vertex.x(), conv_vertex.y(), conv_vertex.z());

            Reco_conv_chi2_probability[Reco_conv_size] = ChiSquaredProbability(conv_vertex.chi2(), conv_vertex.ndof());
            Reco_conv_ntracks[Reco_conv_size] = it->nTracks();

            const std::vector<edm::RefToBase<reco::Track> > conv_tracks = it->tracks();
            
            Reco_conv_trk1_charge[Reco_conv_size] = conv_tracks[0]->charge();
            Reco_conv_trk1_nhit[Reco_conv_size] = conv_tracks[0]->numberOfValidHits();
            Reco_conv_trk1_normalizedChi2[Reco_conv_size] = conv_tracks[0]->normalizedChi2();
            Reco_conv_trk1_theta[Reco_conv_size] = conv_tracks[0]->theta();
            Reco_conv_trk1_phi[Reco_conv_size] = conv_tracks[0]->phi();
            Reco_conv_trk1_d0[Reco_conv_size] = conv_tracks[0]->d0();

            Reco_conv_trk2_charge[Reco_conv_size] = conv_tracks[1]->charge();
            Reco_conv_trk2_nhit[Reco_conv_size] = conv_tracks[1]->numberOfValidHits();
            Reco_conv_trk2_normalizedChi2[Reco_conv_size] = conv_tracks[1]->normalizedChi2();
            Reco_conv_trk2_theta[Reco_conv_size] = conv_tracks[1]->theta();
            Reco_conv_trk2_phi[Reco_conv_size] = conv_tracks[1]->phi();
            Reco_conv_trk2_d0[Reco_conv_size] = conv_tracks[1]->d0();
            
            Reco_conv_dz_innerPosition[Reco_conv_size] = it->tracksInnerPosition()[0].Z() - it->tracksInnerPosition()[1].Z();
            Reco_conv_dPhiTracksAtVtx[Reco_conv_size] = it->dPhiTracksAtVtx();
            Reco_conv_pairCotThetaSeparation[Reco_conv_size] = it->pairCotThetaSeparation();
            Reco_conv_distOfMinimumApproach[Reco_conv_size] = it->distOfMinimumApproach();
            Reco_conv_zOfPrimaryVertexFromTracks[Reco_conv_size] = it->zOfPrimaryVertexFromTracks();
            Reco_conv_pairInvariantMass[Reco_conv_size] = it->pairInvariantMass();

            // std::vector<int> conv_quality_tmp;
            // if (it->quality(reco::Conversion::arbitratedMerged))
            //     conv_quality_tmp.push_back(1);
            // if (it->quality(reco::Conversion::generalTracksOnly))
            //     conv_quality_tmp.push_back(2);
            // if (it->quality(reco::Conversion::arbitratedEcalSeeded))
            //     conv_quality_tmp.push_back(3);
            ...
            //
            Reco_conv_pt[Reco_conv_size] = it->refittedPair4Momentum().Pt();
            Reco_conv_eta[Reco_conv_size] = it->refittedPair4Momentum().Eta();
            Reco_conv_phi[Reco_conv_size] = it->refittedPair4Momentum().Phi();
            Reco_conv_et[Reco_conv_size] = it->refittedPair4Momentum().Et();
            Reco_conv_energy[Reco_conv_size] = it->refittedPair4Momentum().E();
            Reco_conv_deltaEta[Reco_conv_size] = it->tracks()[0]->eta() - it->tracks()[1]->eta();
            Reco_conv_deltaPhi[Reco_conv_size] = deltaPhi( it->tracks()[0]->phi(), it->tracks()[1]->phi() );
            Reco_conv_deltaR[Reco_conv_size] = deltaR( it->tracks()[0]->eta(),
                                                       it->tracks()[0]->phi(),
                                                       it->tracks()[1]->eta(),
                                                       it->tracks()[1]->phi() );
        ...
        std::cout << "Too many conversions: " << collConv->size() << std::endl;
        std::cout << "Maximum allowed: " << Max_conv_size << std::endl;
    }
    //

    if (_fillSingleMuons) this->fillRecoMuons(theCentralityBin);

    if (_useGeTracks) {
        iEvent.getByToken(_recoTracksToken, collTracks);
        if (_fillRecoTracks) this->fillRecoTracks(lastSign);
    }

    this->fillRecoHistos(lastSign);

    if (_fillTree) myTree->Fill();

    return;
}

void HiOniaAnalyzer::fillRecoHistos(int lastSign) {
    // BEST J/PSI?
    if (_onlythebest) {  // yes, fill simply the best (possibly same-sign)
        ...
        for (int iSign = 0; iSign <= lastSign; ++iSign) {
            for (unsigned int count = 0; count < _thePassedCands[iSign].size(); count++) {
                ...
                if (_fillTree) this->fillTreeJpsi(iSign, count);

                for (unsigned int iTr = 0; iTr < NTRIGGERS; ++iTr) {
                    if (isTriggerMatched[iTr]) {
                        this->fillRecoJpsi(
                            iSign, count, theTriggerNames.at(iTr),
                            theCentralities.at(theCentralityBin));
                    }
                }
            }
        }
...
void HiOniaAnalyzer::fillTreeMuon(const pat::Muon *muon, int iType,
                                  ULong64_t trigBits) {
    ...
    if (!_theMinimumFlag) {
        ...
        if (muon->isGlobalMuon()) {
            ...
            Reco_mu_nMuValHits[Reco_mu_size] =
                gTrack->hitPattern().numberOfValidMuonHits();
...
void HiOniaAnalyzer::fillTreeJpsi(int iSign, int count) {
    ...
    for (unsigned int iTr = 1; iTr < NTRIGGERS; ++iTr) {
        ...
        const pat::TriggerObjectStandAloneCollection mu1HLTMatchesFilter =
            muon1->triggerObjectMatchesByFilter(HLTLastFilters[iTr]);
        const pat::TriggerObjectStandAloneCollection mu2HLTMatchesFilter =
            muon2->triggerObjectMatchesByFilter(HLTLastFilters[iTr]);
        if (mu1HLTMatchesFilter.size() > 0) {
            trigBits_mu1 += pow(2, iTr - 1);
        }
        if (mu2HLTMatchesFilter.size() > 0) {
            trigBits_mu2 += pow(2, iTr - 1);
        }
    }
    ...
    if (!(_isHI) && _muonLessPrimaryVertex &&
        aJpsiCand->hasUserData("muonlessPV")) {
        RefVtx = (*aJpsiCand->userData<reco::Vertex>("muonlessPV")).position();
        RefVtx_xError =
            (*aJpsiCand->userData<reco::Vertex>("muonlessPV")).xError();
        RefVtx_yError =
            (*aJpsiCand->userData<reco::Vertex>("muonlessPV")).yError();
        RefVtx_zError =
            (*aJpsiCand->userData<reco::Vertex>("muonlessPV")).zError();
    } else if (!_muonLessPrimaryVertex &&
               aJpsiCand->hasUserData("PVwithmuons")) {
        RefVtx = (*aJpsiCand->userData<reco::Vertex>("PVwithmuons")).position();
        RefVtx_xError =
            (*aJpsiCand->userData<reco::Vertex>("PVwithmuons")).xError();
        RefVtx_yError =
            (*aJpsiCand->userData<reco::Vertex>("PVwithmuons")).yError();
        RefVtx_zError =
            (*aJpsiCand->userData<reco::Vertex>("PVwithmuons")).zError();
    } else {
        cout << "HiOniaAnalyzer::fillTreeJpsi: no PVfor muon pair stored"
             << endl;
        return;
    }

    new ((*Reco_QQ_vtx)[Reco_QQ_size])
        TVector3(RefVtx.X(), RefVtx.Y(), RefVtx.Z());
    ...
    if (muon1->charge() > muon2->charge()) {
        ...
        if (muon1->isGlobalMuon()) gTrack_mupl = muon1->globalTrack();

        if (muon2->isGlobalMuon()) gTrack_mumi = muon2->globalTrack();
    } else {
        ...
        if (muon2->isGlobalMuon()) gTrack_mupl = muon2->globalTrack();

        if (muon1->isGlobalMuon()) gTrack_mumi = muon1->globalTrack();
    }
    ...
    if (!_theMinimumFlag) {
        ...
        if (gTrack_mupl.isNonnull()) {
            Reco_QQ_mupl_nMuValHits[Reco_QQ_size] =
                gTrack_mupl->hitPattern().numberOfValidMuonHits();
            Reco_QQ_mupl_normChi2_global[Reco_QQ_size] =
                gTrack_mupl->normalizedChi2();
        ...
        if (gTrack_mumi.isNonnull()) {
            Reco_QQ_mumi_nMuValHits[Reco_QQ_size] =
                gTrack_mumi->hitPattern().numberOfValidMuonHits();
            Reco_QQ_mumi_normChi2_global[Reco_QQ_size] =
                gTrack_mumi->normalizedChi2();
    ...
    if (_useBS) {
        if (aJpsiCand->hasUserFloat("ppdlBS")) {
            ...
            std::cout << "Warning: User Float ppdlBS was not found"
                      << std::endl;
        }
        if (aJpsiCand->hasUserFloat("ppdlErrBS")) {
            Reco_QQ_ctauErr[Reco_QQ_size] =
                10.0 * aJpsiCand->userFloat("ppdlErrBS");
        } else {
            Reco_QQ_ctauErr[Reco_QQ_size] = -100;
            std::cout << "Warning: User Float ppdlErrBS was not found"
                      << std::endl;
        }
        if (aJpsiCand->hasUserFloat("ppdlBS3D")) {
            Reco_QQ_ctau3D[Reco_QQ_size] =
                10.0 * aJpsiCand->userFloat("ppdlBS3D");
        } else {
            Reco_QQ_ctau3D[Reco_QQ_size] = -100;
            std::cout << "Warning: User Float ppdlBS3D was not found"
                      << std::endl;
        }
        if (aJpsiCand->hasUserFloat("ppdlErrBS3D")) {
            Reco_QQ_ctauErr3D[Reco_QQ_size] =
                10.0 * aJpsiCand->userFloat("ppdlErrBS3D");
        } else {
            Reco_QQ_ctauErr3D[Reco_QQ_size] = -100;
            std::cout << "Warning: User Float ppdlErrBS3D was not found"
                      << std::endl;
        }
    } else {
        if (aJpsiCand->hasUserFloat("ppdlPV")) {
            ...
            Reco_QQ_ctau[Reco_QQ_size] = -100;
            std::cout << "Warning: User Float ppdlPV was not found"
                      << std::endl;
        }
        if (aJpsiCand->hasUserFloat("ppdlErrPV")) {
            Reco_QQ_ctauErr[Reco_QQ_size] =
                10.0 * aJpsiCand->userFloat("ppdlErrPV");
        } else {
            Reco_QQ_ctauErr[Reco_QQ_size] = -100;
            std::cout << "Warning: User Float ppdlErrPV was not found"
                      << std::endl;
        }
        if (aJpsiCand->hasUserFloat("ppdlPV3D")) {
            Reco_QQ_ctau3D[Reco_QQ_size] =
                10.0 * aJpsiCand->userFloat("ppdlPV3D");
        } else {
            Reco_QQ_ctau3D[Reco_QQ_size] = -100;
            std::cout << "Warning: User Float ppdlPV3D was not found"
                      << std::endl;
        }
        if (aJpsiCand->hasUserFloat("ppdlErrPV3D")) {
            Reco_QQ_ctauErr3D[Reco_QQ_size] =
                10.0 * aJpsiCand->userFloat("ppdlErrPV3D");
        } else {
            Reco_QQ_ctau3D[Reco_QQ_size] = -100;
            std::cout << "Warning: User Float ppdlErrPV3D was not found"
                      << std::endl;
        }
    }
    if (_isMC) {
        if (aJpsiCand->hasUserFloat("ppdlTrue")) {
            Reco_QQ_ctauTrue[Reco_QQ_size] =
                10.0 * (float)aJpsiCand->userFloat("ppdlTrue");
        } else {
            Reco_QQ_ctauTrue[Reco_QQ_size] = -100;
            std::cout << "Warning: User Float ppdlTrue was not found"
                      << std::endl;
        }
        if (aJpsiCand->hasUserFloat("ppdlTrue3D")) {
            Reco_QQ_ctauTrue3D[Reco_QQ_size] =
                10.0 * (float)aJpsiCand->userFloat("ppdlTrue3D");
        } else {
            Reco_QQ_ctauTrue3D[Reco_QQ_size] = -100;
            std::cout << "Warning: User Float ppdlTrue3D was not found"
                      << std::endl;
        }
    }
    ...
    if (_useGeTracks && collTracks.isValid()) {
        for (std::vector<reco::Track>::const_iterator it = collTracks->begin();
            ...
            double dzsigma = sqrt(track->dzError() * track->dzError() +
                                  RefVtx_zError * RefVtx_zError);
            double dxy = track->dxy(RefVtx);
            double dxysigma = sqrt(track->dxyError() * track->dxyError() +
                                   RefVtx_xError * RefVtx_yError);
            // to be fixed
            // double dxysigma = sqrt(track->dxyError()*track->dxyError() +
                ...
                if (track->pt() > 0.4) {
                    ...
                    if (iTrack_mupl->charge() == track->charge()) {
                        double Reco_QQ_mupl_NtrkDeltaR =
                            deltaR(iTrack_mupl->eta(), iTrack_mupl->phi(),
                                   track->eta(), track->phi());
                        double Reco_QQ_mupl_RelDelPt =
                            abs(1.0 - iTrack_mupl->pt() / track->pt());
                        ...
                        double Reco_QQ_mumi_NtrkDeltaR =
                            deltaR(iTrack_mumi->eta(), iTrack_mumi->phi(),
                                   track->eta(), track->phi());
                        double Reco_QQ_mumi_RelDelPt =
                            abs(1.0 - iTrack_mumi->pt() / track->pt());
                    ...
                    double Reco_QQ_NtrkDeltaR =
                        deltaR(aJpsiCand->eta(), aJpsiCand->phi(), track->eta(),
                               track->phi());
...
void HiOniaAnalyzer::fillRecoJpsi(int iSign, int count, std::string trigName,
    ...
    if (iSign == 0 && aJpsiCand->mass() >= JpsiMassMin &&
        aJpsiCand->mass() < JpsiMassMax && aJpsiCand->pt() >= JpsiPtMin &&
        aJpsiCand->pt() < JpsiPtMax &&
        fabs(aJpsiCand->rapidity()) >= JpsiRapMin &&
    ...
    if (_fillHistos) {
        if (_combineCategories &&
            _thePassedCats[iSign].at(count) <=
                Trk_Trk) {  // for the moment consider Glb+Glb, GlbTrk+GlbTrk,
            // Trk+Trk
            ...
            switch (_thePassedCats[iSign].at(count)) {
                case Glb_Glb:
                    ...
                    if (isBarrel)
                        myRecoJpsiGlbGlbHistos->Fill(aJpsiCand,
                                                     "Barrel_" + theLabel);
                    else
                        myRecoJpsiGlbGlbHistos->Fill(aJpsiCand,
                                                     "EndCap_" + theLabel);
                ...
                case GlbTrk_GlbTrk:
                    ...
                    if (isBarrel)
                        myRecoJpsiGlbTrkHistos->Fill(aJpsiCand,
                                                     "Barrel_" + theLabel);
                    else
                        myRecoJpsiGlbTrkHistos->Fill(aJpsiCand,
                                                     "EndCap_" + theLabel);
                ...
                case Trk_Trk:
                    ...
                    if (isBarrel)
                        myRecoJpsiTrkTrkHistos->Fill(aJpsiCand,
                                                     "Barrel_" + theLabel);
                    else
                        myRecoJpsiTrkTrkHistos->Fill(aJpsiCand,
                                                     "EndCap_" + theLabel);
...
void HiOniaAnalyzer::fillHistosAndDS(unsigned int theCat,
                                     const pat::CompositeCandidate *aJpsiCand) {
    return;
}

void HiOniaAnalyzer::checkTriggers(const pat::CompositeCandidate *aJpsiCand) {
    const pat::Muon *muon1 =
        dynamic_cast<const pat::Muon *>(aJpsiCand->daughter("muon1"));
    const pat::Muon *muon2 =
        dynamic_cast<const pat::Muon *>(aJpsiCand->daughter("muon2"));
    ...
    for (unsigned int iTr = 1; iTr < NTRIGGERS; ++iTr) {
        ...
        const pat::TriggerObjectStandAloneCollection mu1HLTMatchesPath =
            muon1->triggerObjectMatchesByPath(theTriggerNames.at(iTr), true,
                                              false);
        const pat::TriggerObjectStandAloneCollection mu2HLTMatchesPath =
            muon2->triggerObjectMatchesByPath(theTriggerNames.at(iTr), true,
                                              false);
    ...
    for (unsigned int iTr = 1; iTr < NTRIGGERS; ++iTr) {
        if (isTriggerMatched[iTr]) {
            // fill event counting histogram only once per event, also if
            // several muons fired trigger
            //      if (alreadyFilled[iTr]) continue;
            // since we have bins for event info, let's try to fill here the
            // trigger info for each pair also if there are several pairs
            // matched to the same kind of trigger
...
void HiOniaAnalyzer::makeCuts(int sign) {
...
bool HiOniaAnalyzer::checkCuts(
    const pat::CompositeCandidate *cand, const pat::Muon *muon1,
    const pat::Muon *muon2,
...
pair<unsigned int, const pat::CompositeCandidate *> HiOniaAnalyzer::theBestQQ(
    ...
    pair<unsigned int, const pat::CompositeCandidate *> result =
        make_pair(theBestCat, theBestCand);
    return result;
}
    ...
    if (muonType == (std::string)("GLB")) {
        ...
        return (fabs(aMuon->eta()) < 2.4 &&
                ((fabs(aMuon->eta()) < 1.3 && aMuon->pt() >= 3.3) ||
                 (1.3 <= fabs(aMuon->eta()) && fabs(aMuon->eta()) < 2.2 &&
                  aMuon->p() >= 2.9) ||
...
bool HiOniaAnalyzer::isTightMuon(const pat::Muon *aMuon) {
    return (aMuon->isGlobalMuon() &&
            ...
            aMuon->innerTrack()->hitPattern().trackerLayersWithMeasurement() >
                5);
}

bool HiOniaAnalyzer::selGlobalMuon(const pat::Muon *aMuon) {
    if (!aMuon->isGlobalMuon()) return false;
...
void HiOniaAnalyzer::InitEvent() {
    ...
    _thePassedCats[0].clear();
    _thePassedCands[0].clear();
    _thePassedCats[1].clear();
    _thePassedCands[1].clear();
    _thePassedCats[2].clear();
    ...
    for (std::map<std::string, int>::iterator clearIt =
             mapTriggerNameToIntFired_.begin();
         clearIt != mapTriggerNameToIntFired_.end(); clearIt++) {
        clearIt->second = 0;
    }
    for (std::map<std::string, int>::iterator clearIt =
             mapTriggerNameToPrescaleFac_.begin();
         clearIt != mapTriggerNameToPrescaleFac_.end(); clearIt++) {
        clearIt->second = -1;
    }
...
reco::GenParticleRef HiOniaAnalyzer::findDaughterRef(
    reco::GenParticleRef GenParticleDaughter, int GenParticlePDG) {
    reco::GenParticleRef GenParticleTmp = GenParticleDaughter;
    bool foundFirstDaughter = false;
    for (int j = 0; j < 1000; ++j) {
        if (GenParticleTmp.isNonnull() &&
            GenParticleTmp->numberOfDaughters() > 0) {
            if (GenParticleTmp->pdgId() == GenParticlePDG ||
                GenParticleTmp->daughterRef(0)->pdgId() == GenParticlePDG) {
    ...
    if (GenParticleTmp.isNonnull() &&
        (GenParticleTmp->pdgId() == GenParticlePDG)) {
...
void HiOniaAnalyzer::fillGenInfo() {
    ...
    if (collGenParticles.isValid()) {
        for (std::vector<reco::GenParticle>::const_iterator it =
                 collGenParticles->begin();
             it != collGenParticles->end(); ++it) {
            const reco::GenParticle *gen = &(*it);

            if (abs(gen->pdgId()) == _oniaPDG &&
                (gen->status() == 2 ||
                 (abs(gen->pdgId()) == 23 && gen->status() == 62)) &&
                gen->numberOfDaughters() >= 2) {
                reco::GenParticleRef genMuon1 =
                    findDaughterRef(gen->daughterRef(0), gen->pdgId());
                reco::GenParticleRef genMuon2 =
                    findDaughterRef(gen->daughterRef(1), gen->pdgId());

                if (abs(genMuon1->pdgId()) == 13 &&
                    abs(genMuon2->pdgId()) == 13 && (genMuon1->status() == 1) &&
                    (genMuon2->status() == 1)) {
                    Gen_QQ_type[Gen_QQ_size] =
                        _isPromptMC ? 0 : 1;  // prompt: 0, non-prompt: 1
                    std::pair<int, std::pair<float, float>> MCinfo =
                        findGenMCInfo(gen);
                    ...
                    if (genMuon1->charge() > genMuon2->charge()) {
                        new ((*Gen_QQ_mupl_4mom)[Gen_QQ_size])
                            TLorentzVector(vMuon1);
                        new ((*Gen_QQ_mumi_4mom)[Gen_QQ_size])
...
void HiOniaAnalyzer::fillRecoTracks(int lastSign) {
        ...
        for (std::vector<reco::Track>::const_iterator it = collTracks->begin();
            ...
            if (track->qualityByName("highPurity") && track->pt() > 0.2 &&
                ...
                if (Reco_trk_size >= Max_trk_size) {
                    std::cout << "Too many tracks: " << Reco_trk_size
                              << std::endl;
                    std::cout << "Maximum allowed: " << Max_trk_size
                              << std::endl;
                    break;
                }
                if (fabs(dz / dzsigma) < 3.0 && fabs(dxy / dxysigma) < 3.0)
                    Reco_isgoodTrk[Reco_trk_size] = true;
                else
                    Reco_isgoodTrk[Reco_trk_size] = false;

                Reco_isMuTrk[Reco_trk_size] = false;

                for (int iSign = 0; iSign <= lastSign; ++iSign) {
                    for (unsigned int count = 0;
                         count < _thePassedCands[iSign].size(); count++) {
                        // Get muon{{{
                        const pat::CompositeCandidate *aJpsiCand =
                            _thePassedCands[iSign].at(count);
                        const pat::Muon *muon1 =
                            dynamic_cast<const pat::Muon *>(
                                aJpsiCand->daughter("muon1"));
                        const pat::Muon *muon2 =
                            dynamic_cast<const pat::Muon *>(
                                aJpsiCand->daughter("muon2"));
                        ...
                        if (tTrack_mupl->charge() == track->charge()) {
                            double Reco_QQ_mupl_NtrkDeltaR =
                                deltaR(tTrack_mupl->eta(), tTrack_mupl->phi(),
                                       track->eta(), track->phi());
                            double Reco_QQ_mupl_RelDelPt =
                                abs(1.0 - tTrack_mupl->pt() / track->pt());
                            if (Reco_QQ_mupl_NtrkDeltaR < 0.001 &&
                                Reco_QQ_mupl_RelDelPt < 0.001)
                                Reco_isMuTrk[Reco_trk_size] = true;
                        } else {
                            double Reco_QQ_mumi_NtrkDeltaR =
                                deltaR(tTrack_mumi->eta(), tTrack_mumi->phi(),
                                       track->eta(), track->phi());
                            double Reco_QQ_mumi_RelDelPt =
                                abs(1.0 - tTrack_mumi->pt() / track->pt());
                            if (Reco_QQ_mumi_NtrkDeltaR < 0.001 &&
                                Reco_QQ_mumi_RelDelPt < 0.001)
                                Reco_isMuTrk[Reco_trk_size] = true;
                        }
                ...
                new ((*Reco_trk_vtx)[Reco_trk_size])
                    TVector3(track->vx(), track->vy(), track->vz());

                Reco_trk_dxyError[Reco_trk_size] = track->dxyError();
                Reco_trk_dzError[Reco_trk_size] = track->dzError();

                TLorentzVector vTrack;
                vTrack.SetPtEtaPhiM(track->pt(), track->eta(), track->phi(),
                                    0.13957018);
...
void HiOniaAnalyzer::fillRecoMuons(int iCent) {
    ...
    if (collMuon.isValid()) {
        for (vector<pat::Muon>::const_iterator it = collMuon->begin();
            ...
            std::string theLabel =
                theTriggerNames.at(0) + "_" + theCentralities.at(iCent);

            if (_fillHistos) {
                if (_combineCategories) {
                    if ((muon->isGlobalMuon() && selGlobalMuon(muon)) ||
                        (muon->isTrackerMuon() && selTrackerMuon(muon))) {
                    ...
                    if (muon->isGlobalMuon() && selGlobalMuon(muon)) {
                        ...
                        if (isBarrel)
                            myRecoGlbMuonHistos->Fill(muon,
                                                      "Barrel_" + theLabel);
                        else
                            myRecoGlbMuonHistos->Fill(muon,
                                                      "EndCap_" + theLabel);
                    } else if (muon->isTrackerMuon() && selTrackerMuon(muon)) {
                        myRecoTrkMuonHistos->Fill(muon, "All_" + theLabel);
                        if (isBarrel)
                            myRecoTrkMuonHistos->Fill(muon,
                                                      "Barrel_" + theLabel);
                        else
                            myRecoTrkMuonHistos->Fill(muon,
                                                      "EndCap_" + theLabel);
            ...
            if (muType == Glb || muType == GlbTrk || muType == Trk) {
                ...
                for (unsigned int iTr = 1; iTr < NTRIGGERS; ++iTr) {
                    const pat::TriggerObjectStandAloneCollection
                        muHLTMatchesFilter = muon->triggerObjectMatchesByFilter(
                            HLTLastFilters[iTr]);
                    const pat::TriggerObjectStandAloneCollection
                        muHLTMatchesPath = muon->triggerObjectMatchesByPath(
                            theTriggerNames.at(iTr), true, false);

                    // apparently matching by path gives false positives so we
                    // use matching by filter for all triggers for which we know
                    // the filter name
                    if (muHLTMatchesFilter.size() > 0) {
                        std::string theLabel = theTriggerNames.at(iTr) + "_" +
                                               theCentralities.at(iCent);

                        if (_fillHistos) {
                            if (_combineCategories) {
                                myRecoMuonHistos->Fill(muon, "All_" + theLabel);
                                if (isBarrel)
                                    myRecoMuonHistos->Fill(
                                        muon, "Barrel_" + theLabel);
                                else
                                    myRecoMuonHistos->Fill(
                                        muon, "EndCap_" + theLabel);
                            } else if (muType == Glb || muType == GlbTrk) {
                                myRecoGlbMuonHistos->Fill(muon,
                                                          "All_" + theLabel);
                                if (isBarrel)
                                    myRecoGlbMuonHistos->Fill(
                                        muon, "Barrel_" + theLabel);
                                else
                                    myRecoGlbMuonHistos->Fill(
                                        muon, "EndCap_" + theLabel);
                ...
                if (_fillTree) this->fillTreeMuon(muon, muType, trigBits);
            }
        }
    }
...
void HiOniaAnalyzer::InitTree() {
    ...
    Reco_conv_vertex = new TClonesArray("TVector3", 100);
    ...
    myTree->Branch("Reco_conv_vertex", "TClonesArray", &Reco_conv_vertex, 32000, 0);

    myTree->Branch("Reco_conv_chi2_probability", Reco_conv_chi2_probability, "Reco_conv_chi2_probability[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_ntracks", Reco_conv_ntracks, "Reco_conv_ntracks[Reco_conv_size]/I");
    myTree->Branch("Reco_conv_pairInvariantMass", Reco_conv_pairInvariantMass, "Reco_conv_pairInvariantMass[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_pairCotThetaSeparation", Reco_conv_pairCotThetaSeparation, "Reco_conv_pairCotThetaSeparation[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_dz_innerPosition", Reco_conv_dz_innerPosition, "Reco_conv_dz_innerPosition[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_dPhiTracksAtVtx", Reco_conv_dPhiTracksAtVtx, "Reco_conv_dPhiTracksAtVtx[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_distOfMinimumApproach", Reco_conv_distOfMinimumApproach, "Reco_conv_distOfMinimumApproach[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_zOfPrimaryVertexFromTracks", Reco_conv_zOfPrimaryVertexFromTracks, "Reco_conv_zOfPrimaryVertexFromTracks[Reco_conv_size]/D");
    //
    myTree->Branch("Reco_conv_trk1_charge", Reco_conv_trk1_charge, "Reco_conv_trk1_charge[Reco_conv_size]/I");
    myTree->Branch("Reco_conv_trk1_nhit", Reco_conv_trk1_nhit, "Reco_conv_trk1_nhit[Reco_conv_size]/I");
    myTree->Branch("Reco_conv_trk1_normalizedChi2", Reco_conv_trk1_normalizedChi2, "Reco_conv_trk1_normalizedChi2[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_trk1_theta", Reco_conv_trk1_theta, "Reco_conv_trk1_theta[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_trk1_phi", Reco_conv_trk1_phi, "Reco_conv_trk1_phi[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_trk1_d0", Reco_conv_trk1_d0, "Reco_conv_trk1_d0[Reco_conv_size]/D");
    //
    myTree->Branch("Reco_conv_trk2_charge", Reco_conv_trk2_charge, "Reco_conv_trk2_charge[Reco_conv_size]/I");
    myTree->Branch("Reco_conv_trk2_nhit", Reco_conv_trk2_nhit, "Reco_conv_trk2_nhit[Reco_conv_size]/I");
    myTree->Branch("Reco_conv_trk2_normalizedChi2", Reco_conv_trk2_normalizedChi2, "Reco_conv_trk2_normalizedChi2[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_trk2_theta", Reco_conv_trk2_theta, "Reco_conv_trk2_theta[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_trk2_phi", Reco_conv_trk2_phi, "Reco_conv_trk2_phi[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_trk2_d0", Reco_conv_trk2_d0, "Reco_conv_trk2_d0[Reco_conv_size]/D");
    //
    myTree->Branch("Reco_conv_pt", Reco_conv_pt, "Reco_conv_pt[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_eta", Reco_conv_eta, "Reco_conv_eta[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_phi", Reco_conv_phi, "Reco_conv_phi[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_et", Reco_conv_et, "Reco_conv_et[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_energy", Reco_conv_energy, "Reco_conv_energy[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_deltaEta", Reco_conv_deltaEta, "Reco_conv_deltaEta[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_deltaPhi", Reco_conv_deltaPhi, "Reco_conv_deltaPhi[Reco_conv_size]/D");
    myTree->Branch("Reco_conv_deltaR", Reco_conv_deltaR, "Reco_conv_deltaR[Reco_conv_size]/D");
    //
...
void HiOniaAnalyzer::beginJob() {
    ...
    // book histos
    // hGoodMuonsNoTrig = new
    // TH1F("hGoodMuonsNoTrig","hGoodMuonsNoTrig",10,0,10);
    hGoodMuonsNoTrig =
        fs->make<TH1F>("hGoodMuonsNoTrig", "hGoodMuonsNoTrig", 10, 0, 10);
        ...
        for (unsigned int j = 0; j < NTRIGGERS; ++j) {
            for (unsigned int k = 0; k < theCentralities.size(); ++k) {
                ...
                if (_combineCategories) {
                    ...
                    myRecoMuonHistos->GetHistograms(theAppendix)
                        ->SetMassBinning(1, 0.10, 0.11);
                    myRecoMuonHistos->GetHistograms(theAppendix)
                        ->SetPtBinning(200, 0.0, 100.0);
                } else {
                    myRecoGlbMuonHistos->Add(theAppendix);
                    myRecoTrkMuonHistos->Add(theAppendix);

                    myRecoGlbMuonHistos->GetHistograms(theAppendix)
                        ->SetMassBinning(1, 0.10, 0.11);
                    myRecoGlbMuonHistos->GetHistograms(theAppendix)
                        ->SetPtBinning(200, 0.0, 100.0);

                    myRecoTrkMuonHistos->GetHistograms(theAppendix)
                        ->SetMassBinning(1, 0.10, 0.11);
                    myRecoTrkMuonHistos->GetHistograms(theAppendix)
                        ->SetPtBinning(200, 0.0, 100.0);
                }
                ...
                for (unsigned int l = 0; l < theSign.size(); ++l) {
                    // J/psi
                    if (_combineCategories)
                        myRecoJpsiHistos->Add(theAppendix + "_" +
                                              theSign.at(l));
                    else {
                        myRecoJpsiGlbGlbHistos->Add(theAppendix + "_" +
                                                    theSign.at(l));
                        myRecoJpsiGlbTrkHistos->Add(theAppendix + "_" +
                                                    theSign.at(l));
                        myRecoJpsiTrkTrkHistos->Add(theAppendix + "_" +
                                                    theSign.at(l));
                    }
                }
            }
    ...
    */
    // hStats = new TH1F("hStats","hStats;;Number of
    // Events",2*NTRIGGERS+1,0,2*NTRIGGERS+1);
    hStats = fs->make<TH1F>("hStats", "hStats;;Number of Events",
                            2 * NTRIGGERS + 1, 0, 2 * NTRIGGERS + 1);
    hStats->GetXaxis()->SetBinLabel(1, "All");
    for (int i = 2; i < (int)theTriggerNames.size() + 1; ++i) {
        hStats->GetXaxis()->SetBinLabel(
            i, theTriggerNames.at(i - 1).c_str());  // event info
        hStats->GetXaxis()->SetBinLabel(
            i + NTRIGGERS,
            theTriggerNames.at(i - 1).c_str());  // muon pair info
    }
    hStats->Sumw2();

    // hCent = new TH1F("hCent","hCent;centrality bin;Number of
    // Events",200,0,200);
    hCent = fs->make<TH1F>("hCent", "hCent;centrality bin;Number of Events",
                           200, 0, 200);
    hCent->Sumw2();

    // hPileUp = new TH1F("hPileUp","Number of Primary Vertices;n_{PV};counts",
    // 50, 0, 50);
    hPileUp = fs->make<TH1F>(
        "hPileUp", "Number of Primary Vertices;n_{PV};counts", 50, 0, 50);
    hPileUp->Sumw2();

    // hZVtx = new TH1F("hZVtx","Primary z-vertex distribution;z_{vtx}
    // [cm];counts", 120, -30, 30);
    hZVtx = fs->make<TH1F>("hZVtx",
                           "Primary z-vertex distribution;z_{vtx} [cm];counts",
                           120, -30, 30);
    hZVtx->Sumw2();
...
void HiOniaAnalyzer::beginRun(const edm::Run &iRun,
                              const edm::EventSetup &iSetup) {
    // init HLTConfigProvider

    EDConsumerBase::Labels labelTriggerResults;
    EDConsumerBase::labelsForToken(_tagTriggerResultsToken,
                                   labelTriggerResults);
    const std::string pro = labelTriggerResults.process;

    // bool init(const edm::Run& iRun, const edm::EventSetup& iSetup, const
    // std::string& processName, bool& changed);
    bool changed = true;
    hltConfigInit = false;
    ...
    if (hltPrescaleProvider.init(iRun, iSetup, pro, changed))
        hltPrescaleInit = true;
...
void HiOniaAnalyzer::endJob() {
    ...
    std::cout << "Total number of passed candidates = " << passedCandidates
              << std::endl;
    /*
    fOut->cd();
    hStats->Write();
...
TLorentzVector HiOniaAnalyzer::lorentzMomentum(
    const reco::Candidate::LorentzVector &p) {
    TLorentzVector res;
...
void HiOniaAnalyzer::hltReport(const edm::Event &iEvent,
                               const edm::EventSetup &iSetup) {
    std::map<std::string, bool> mapTriggernameToTriggerFired;
    std::map<std::string, unsigned int> mapTriggernameToHLTbit;

    for (std::vector<std::string>::const_iterator it = theTriggerNames.begin();
         it != theTriggerNames.end(); ++it) {
    ...
    if (hltConfigInit) {
        ...
        for (std::map<std::string, unsigned int>::iterator it =
                 mapTriggernameToHLTbit.begin();
             it != mapTriggernameToHLTbit.end(); it++) {
            unsigned int triggerIndex = hltConfig.triggerIndex(it->first);
            if (it->first == "NoTrigger") continue;
            if (triggerIndex >= n) {
                std::cout << "[HiOniaAnalyzer::hltReport] --- TriggerName "
                          << it->first << " not available in config!"
                          << std::endl;
            } else {
                it->second = triggerIndex;
                //      std::cout << "[HiOniaAnalyzer::hltReport] ---
                //      TriggerName " << it->first << " available in config!" <<
                //      std::endl;
            }
        }
    }
    ...
    try {
        ...
        //    std::cout << "[HiOniaAnalyzer::hltReport] --- J/psi TriggerResult
        //    is present in current event" << std::endl;
    } catch (...) {
        //    std::cout << "[HiOniaAnalyzer::hltReport] --- J/psi TriggerResults
        //    NOT present in current event" << std::endl;
    }
    if (collTriggerResults.isValid() &&
        (collTriggerResults->size() == hltConfig.size())) {
        //    std::cout << "[HiOniaAnalyzer::hltReport] --- J/psi TriggerResults
        //    IS valid in current event" << std::endl;

        // loop over Trigger Results to check if paths was fired
        for (std::vector<std::string>::iterator itHLTNames =
                 theTriggerNames.begin();
             itHLTNames != theTriggerNames.end(); itHLTNames++) {
            const std::string triggerPathName = *itHLTNames;
            if (mapTriggernameToHLTbit[triggerPathName] < 1000) {
                if (collTriggerResults->accept(
                        mapTriggernameToHLTbit[triggerPathName])) {
                ...
                if (_isMC) {
                    ...
                    //-------prescale factor------------
                    if (hltPrescaleInit &&
                        hltPrescaleProvider.prescaleSet(iEvent, iSetup) >= 0) {
                        std::pair<std::vector<std::pair<std::string, int>>, int>
                            detailedPrescaleInfo =
                                hltPrescaleProvider.prescaleValuesInDetail(
                                    iEvent, iSetup, triggerPathName);
                        ...
                        if (detailedPrescaleInfo.first.size() == 1) {
                            l1Prescale =
                                detailedPrescaleInfo.first.at(0).second;
                        } else if (detailedPrescaleInfo.first.size() > 1) {
                            l1Prescale =
                                1;  // Means it is a complex l1 seed, and for us
                                    // it is only Mu3 OR Mu5, both of them
                                    // unprescaled at L1
                        } else {
                            std::cout
                                << "[HiOniaAnalyzer::hltReport] --- L1 "
                                   "prescale was NOT found for TriggerName "
                                << triggerPathName
                                << " , default L1 prescale value set to 1 "
                                << std::endl;
                        }
                        // compute the total prescale = HLT prescale * L1
                        // prescale
                        mapTriggerNameToPrescaleFac_[triggerPathName] =
                            hltPrescale * l1Prescale;
                    }
                }
            }
        }
    } else
        std::cout << "[HiOniaAnalyzer::hltReport] --- TriggerResults NOT valid "
                     "in current event"
                  << std::endl;
...
bool HiOniaAnalyzer::isAbHadron(int pdgID) {
    if (abs(pdgID) == 511 || abs(pdgID) == 521 || abs(pdgID) == 531 ||
        abs(pdgID) == 5122)
        return true;
    return false;
}
...
reco::GenParticleRef HiOniaAnalyzer::findMotherRef(
    reco::GenParticleRef GenParticleMother, int GenParticlePDG) {
    for (int i = 0; i < 1000; ++i) {
        if (GenParticleMother.isNonnull() &&
            (GenParticleMother->pdgId() == GenParticlePDG) &&
...
std::pair<int, std::pair<float, float>> HiOniaAnalyzer::findGenMCInfo(
    ...
    if (genJpsi->numberOfMothers() > 0) {
        ...
        trueVtx.SetXYZ(genJpsi->vertex().x(), genJpsi->vertex().y(),
                       genJpsi->vertex().z());
        trueP.SetXYZ(genJpsi->momentum().x(), genJpsi->momentum().y(),
                     genJpsi->momentum().z());

        bool aBhadron = false;
        reco::GenParticleRef Jpsimom =
            findMotherRef(genJpsi->motherRef(), genJpsi->pdgId());
        if (Jpsimom.isNull()) {
            std::pair<float, float> trueLifePair =
                std::make_pair(trueLife, trueLife3D);
            std::pair<int, std::pair<float, float>> result =
                std::make_pair(momJpsiID, trueLifePair);
            return result;
        } else if (Jpsimom->numberOfMothers() <= 0) {
            if (isAbHadron(Jpsimom->pdgId())) {
                momJpsiID = Jpsimom->pdgId();
                trueVtxMom.SetXYZ(Jpsimom->vertex().x(), Jpsimom->vertex().y(),
                                  Jpsimom->vertex().z());
                aBhadron = true;
            }
        } else {
            reco::GenParticleRef Jpsigrandmom =
                findMotherRef(Jpsimom->motherRef(), Jpsimom->pdgId());
            if (isAbHadron(Jpsimom->pdgId())) {
                if (Jpsigrandmom.isNonnull() &&
                    isAMixedbHadron(Jpsimom->pdgId(), Jpsigrandmom->pdgId())) {
                    momJpsiID = Jpsigrandmom->pdgId();
                    trueVtxMom.SetXYZ(Jpsigrandmom->vertex().x(),
                                      Jpsigrandmom->vertex().y(),
                                      Jpsigrandmom->vertex().z());
                } else {
                    momJpsiID = Jpsimom->pdgId();
                    trueVtxMom.SetXYZ(Jpsimom->vertex().x(),
                                      Jpsimom->vertex().y(),
                                      Jpsimom->vertex().z());
                }
                aBhadron = true;
            } else if (Jpsigrandmom.isNonnull() &&
                       isAbHadron(Jpsigrandmom->pdgId())) {
                if (Jpsigrandmom->numberOfMothers() <= 0) {
                    ...
                    reco::GenParticleRef JpsiGrandgrandmom = findMotherRef(
                        Jpsigrandmom->motherRef(), Jpsigrandmom->pdgId());
                    if (JpsiGrandgrandmom.isNonnull() &&
                        isAMixedbHadron(Jpsigrandmom->pdgId(),
                                        JpsiGrandgrandmom->pdgId())) {
                        momJpsiID = JpsiGrandgrandmom->pdgId();
                        trueVtxMom.SetXYZ(JpsiGrandgrandmom->vertex().x(),
                                          JpsiGrandgrandmom->vertex().y(),
                                          JpsiGrandgrandmom->vertex().z());
                    } else {
                        momJpsiID = Jpsigrandmom->pdgId();
                        trueVtxMom.SetXYZ(Jpsigrandmom->vertex().x(),
                                          Jpsigrandmom->vertex().y(),
...
pat::CompositeCandidate *HiOniaAnalyzer::makeconvCandidate(const reco::Conversion &reco_conv) {
    ...
    math::XYZTLorentzVector p4_ele1 = math::XYZTLorentzVector(
        mom_ele1.X(), mom_ele1.Y(), mom_ele1.Z(), E_ele1);
    ele1->setP4(p4_ele1);
    ...
    math::XYZTLorentzVector p4_ele2 = math::XYZTLorentzVector(
        mom_ele2.X(), mom_ele2.Y(), mom_ele2.Z(), E_ele2);
    ele2->setP4(p4_ele2);
